...
- Menuepunkt "Kalender": links eine Leiste mit allen Teammitgliedern (wird automatisch aktualisiert, sobald jemand im Team hinzugefuegt wird); pro Person ein Formular fuer einen neuen Zeitraum (Datum+Uhrzeit von/bis, optional ein Titel/Grund) mit "Hinzufuegen"-Button, darunter ein aufklappbares Dropdown mit allen bisher hinterlegten Zeitraeumen dieser Person (jeweils mit Loeschen-Button). Rechts der eigentliche Kalender in der Farbe der jeweiligen Person markiert, umschaltbar zwischen Monats-, Wochen- und Jahres-Ansicht (Wochen-Ansicht zeigt die Uhrzeiten zusaetzlich als Balken auf einer Zeitachse; Jahres-Ansicht zeigt 12 Mini-Monate mit kleinen Punkten je Tag, Klick auf einen Monatsnamen springt in die Monats-Ansicht); Navigation vor/zurueck sowie ein "Heute"-Button
- Im Kalender-Formular gibt es statt freier Von/Bis-Eingabe auch zwei Vorlagen: "Rufbereitschaft Wochentags" (5 Bloecke: Mo 00:01-08:30 sowie taeglich abends 18:00 bis zum naechsten Morgen 08:30, bis Fr 08:30 - deckt nur Naechte/Randzeiten ab, nicht die Arbeitszeit tagsueber) und "Rufbereitschaft Wochenende" (Fr 18:00 - So 23:59); es reicht, ein beliebiges Datum in der Zielwoche anzugeben. Zusaetzlich laesst sich jeder Zeitraum (frei gewaehlt oder Vorlage) woechentlich wiederholen, bis zu einem gewaehlten Enddatum. Alle so auf einmal angelegten Zeitraeume gehoeren zu einer "Serie" und lassen sich in der Liste entweder einzeln oder alle zusammen mit einem Klick wieder entfernen
...
- Jeder einzelne Tag kann zusaetzlich eine freie Notiz/Bemerkung bekommen: eine kleine Sprechblase unten rechts am Tag oeffnet ein Notizfeld; ohne Notiz taucht sie nur beim Ueberfahren des Tags dezent auf, sobald eine Notiz hinterlegt ist bleibt sie permanent sichtbar (gefuellt) und zeigt den Text beim Ueberfahren als Tooltip
...
└── frontend/
    ...
    │       ├── CalendarPage.jsx # Kalender-Seite (Team-Leiste + Monats-/Wochen-Ansicht)
    │       ├── TeamMemberPanel.jsx # Ein Teammitglied in der Kalender-Leiste (Formular + Eintraege)
    │       ├── CalendarMonthView.jsx # Monats-Raster fuer den Kalender
    │       ├── CalendarWeekView.jsx  # Wochen-Ansicht mit Zeitachse fuer den Kalender
    │       ├── CalendarYearView.jsx  # Jahres-Ansicht (12 Mini-Monate) fuer den Kalender
